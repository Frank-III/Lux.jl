...
@generated function _get_batch_statistics(x::AbstractArray,
        running_mean::R,
        running_var::R,
        r::Val{reduce_dims},
        ::Val{training},
    ...
    if !training
        ...
        if R != Nothing
            push!(calls,
                :(_stats = _update_normalization_statistics(x,
                    running_mean,
                    running_var,
                    batchmean,
                    batchvar,
...
@generated function _affine_normalize(x::AbstractArray,
        xmean::ST,
        xvar::ST,
        scale::A,
        bias::A,
        epsilon::Real) where {ST, A}
    if A != Nothing
...
function _normalization_impl(x::AbstractArray,
        running_mean::R,
        running_var::R,
        scale::A,
        bias::A,
...
function _normalization(x::AbstractArray,
        running_mean::Union{AbstractVector, Nothing},
    ...
    x_, rm, rv = _normalization_impl(x,
        rm_,
        rv_,
        s_,
        b_,
